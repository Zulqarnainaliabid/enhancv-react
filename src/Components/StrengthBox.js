...
import { INCREMENT , INCREMENTBACKGROUNDCOLORSTRENGTH , INDUXSTRENGTH, SETTOGGLEBUTTONNULL,} from "./Redux/actions/indux";
...
export default function Boxfunction(props) {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem("arrayStrength") !== null) {
      ...
      setBullotsTextHolder(value[props.index].value.DiscriptionTextHolder);
      let array = [];
  ...
  useEffect(() => {
    if(UpdateWidthLeftRight!==null){
      UpdateWidthLeftRight.map((item,index)=>{
        if(UpdateWidthLeftRight[index].name==="Strength")
        {
        ...
        <div style={{ display: "flex" }}>
          ...
          >
            <input
              ...
              placeholder="Your Unique Tenlent"
            />
            <div
              onClick={handleText}
            ...
            >
              {checkplacehoderBollets ? (
                ...
                <Editor
                  ...
                  onChange={(text) => {
                    let array = props.list;
                    array[props.index].value.titleSocialnetwork = text;
