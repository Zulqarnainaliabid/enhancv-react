...
export default function Boxfunction (props) {
  ...
  async function GetData () {
    let value = await localStorage.getItem ('Section');
    value = JSON.parse (value);
    return value;
  }
  ...
  async function HandleWidth () {
    ...
    if (data !== null) {
      ...
      if (JSON.parse (localStorage.getItem ('Singlecolumn'))) {
        setToggleWidth (false);
      } else {
        for (let i = 0; i < data.length; i++) {
          if (data[i].name === 'Find Me') {
            if (data[i].Left) {
              setToggleWidth (true);
            } else {
              setToggleWidth (false);
  ...
  const style = {
    ...
    width: JSON.parse (localStorage.getItem ('Singlecolumn')) ? '835px' : '87%',
  };
  ...
  console.log ('setToggleWidth', ToggleWidth);
  return (
    <div>
      <div
        style={{
      ...
      >
        <div style={{display: 'flex', width: '100%'}}>
          ...
          <div
            className="outerWraperInputFieldHeaderFindMeOnline"
            onClick={() => {
              setLinkTextBox ('none');
              setToggleButtons (false);
              setlistIcon (false);
            }}
            style={ToggleWidth ? styleToggleed : style}
          >
            <input
              ref={inputref}
              type="text"
