        ...
        onKeyDown={e => {
          if (e.keyCode === 8) {
            if (e.target.value.length === 0) {
              props.HandlerMinusToolAndTechnology ();
            }
          }
          if (e.key === 'Enter') {
            if (e.target.value.length > 0) {
              props.HandlerAddingToolAndTechnology ();
            }
          }
        }}
        onChange={e => {
...
export default function Boxfunction (props) {
  ...
  const [BorderStyle, setBorderStyle] = useState (
    'ToolAndTechnologyTechStockSection'
  );
  ...
  function HandleSetBorder () {
    ...
    localStorage.setItem (
      'arrayTechStockSetBorder',
      JSON.stringify ('ToolAndTechnologyTechStockSectionBorder')
    );
  }
  return (
    <div>
              ...
              return (
                <div>
                  ...
                  <div className="outerWraperBorderStyle">
                    ...
                    <div
                      style={{display: 'flex'}}
                      className="RadioButtonsBorderStyle"
                    >
                      <div
                      ...
                      <div
                        onClick={() => {
                          ...
                          localStorage.setItem (
                            'arrayTechStockSetBorder',
                            JSON.stringify ('ToolAndTechnologyTechStockSection')
                          );
                        }}
                        className="RadioButtonsToggleButtons"
        ...
        >
          <div style={{display: ShowGroupName ? 'block' : 'none'}}>
            <input
              ...
              className={
                UpdateColor
                  ? 'companyTitleExperienceSectionBlue'
                  : 'companyTitleExperienceSectionGreen'
              }
          ...
          <div style={{display: 'flex', flexWrap: 'wrap'}}>
            {ArrayTechnology[
              props.index
            ].Texttoolandtechnology.map ((item, index) => {
              return (
                <ToolAndTechnology
                  HandlerAddingToolAndTechnology={
                    HandlerAddingToolAndTechnology
                  }
                  HandlerMinusToolAndTechnology={HandlerMinusToolAndTechnology}
                  key={index}
                  index={index}
