...
export default function PhilosophyOuterSection (props) {
  ...
  function GetItemFindMeOnline () {
    let value = localStorage.getItem ('Philosophy');
  ...
  useEffect (() => {
    let value = localStorage.getItem ('HeadingValuePhilosophy');
    value = JSON.parse (value);
  ...
  useEffect (
    () => {
      if (contextData.BackGroundHighLitter !== 'Philosophy') {
        setHighLighter (false);
        handleInnerHight ();
      }
    },
    ...
    <div>
      <div
        ...
        onClick={e => {
          ...
          contextData.HandleUpdateBackGroundHighLitter ('Philosophy');
        }}
      >
        <div
