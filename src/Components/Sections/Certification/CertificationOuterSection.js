...
export default function CertificationOuterSection (props) {
  ...
  function GetItemFindMeOnline () {
    let value = localStorage.getItem ('Certification');
  ...
  useEffect (() => {
    let value = localStorage.getItem ('HeadingValueCertification');
    value = JSON.parse (value);
  ...
  useEffect (
    () => {
      if (contextData.BackGroundHighLitter !== 'Certification') {
        setHighLighter (false);
        handleInnerHight ();
      }
    },
    ...
    <div>
      <div
        ...
        onClick={e => {
          ...
          contextData.HandleUpdateBackGroundHighLitter ('Certification');
        }}
      >
        <div
