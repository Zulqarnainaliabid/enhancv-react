		...
		<>
			<div style={{ position: 'relative' }}>
				<div style={{ display: ShowHeaderUnderButton }} className="headingOptionUnderAchievementSection">
					...
					<div  className="ArrangeIcon" onClick={()=>{
				...
				{listIcon &&
					...
					{iconListData && iconListData.map((item)=>{
                     return<div className="iconsList" onClick={()=>{
						setIcon(item.icon)
					 }} >{item.icon}</div>
					})}
					</div>
			...
			>
				...
				<div  style={{display:ShowGroupName}} className="Icon">{Icon}</div>
				<input className="outerWraperGroupTitle"
				placeholder="What are you most proud of?" />
				</div>
				<div  style={{display:ShowDiscription}}>
				<MyStatefulEditor data={"What are you proud in this role"} />
				</div>
			</div>
		</>
	);
		...
		<>
		   <div style={{position:"relative"}}>
			...
			<div style={{ display: 'flex', flexDirection: 'column' }}>
				...
				>
					<div style={{ borderBottom: '4px solid' }}>Achievements</div>
				</div>
